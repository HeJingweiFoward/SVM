package distributedSVM.cascade;

import java.io.IOException;
import java.util.Random;
import java.util.StringTokenizer;
import java.util.Vector;
import distributedSVM.cascade.io.TrainingSubsetInputFormat;
...
//		prepartitionConfs[0] .set("mapreduce.app-submission.cross-platform", "true");    
//		prepartitionConfs[0] .set("mapreduce.job.jar", "e:\\2\\SVMCas.jar"); 
//		prepartitionConfs[1] .set("mapreduce.app-submission.cross-platform", "true");    
//		prepartitionConfs[1] .set("mapreduce.job.jar", "e:\\2\\SVMCas.jar"); 
		...
		prepartitionJobs[0] = Job.getInstance(prepartitionConfs[0], "Cascade SVM: Partitioning training data, Phase 1");
		...
		prepartitionJobs[1] = Job.getInstance(prepartitionConfs[1], "Cascade SVM: Partitioning training data, Phase 2");
		...
		String cacheURI = "hdfs://datanode1:9000/SVM/libsvm.jar";

		for(int jobItr = 0; jobItr < cascadeJobCount; jobItr++){
			cascadeConfs[jobItr] =  new Configuration();		
//			cascadeConfs[jobItr] .set("mapreduce.app-submission.cross-platform", "true");    
//			cascadeConfs[jobItr] .set("mapreduce.job.jar", "e:\\2\\SVMCas.jar"); 
			if(jobItr == cascadeJobCount-1) {
				//the last layer's reducer will need the output path string to write final model to HDFS
				cascadeConfs[jobItr].set("USER_OUTPUT_PATH", otherArgs[1]);	
			}
			cascadeJobs[jobItr] = Job.getInstance(cascadeConfs[jobItr], "Cascade SVM: Layer " + (jobItr+1));
			cascadeJobs[jobItr].addFileToClassPath(new Path(cacheURI));	
		...
		System.out.println("===== Beginning job for predict  =====");
		
		svm_hdfs_predict.main(new String[] {"/SVM/breastCancerTest","/SVM/BCmodel","/SVM/predict"});
